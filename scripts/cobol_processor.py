...
def browse_directory_files():
    ...
    if cpy_files:
        if len(cpy_files) == 1:
            ...
            cpy_choices = [
                {"name": os.path.basename(f), "value": f, "description": f}
                for f in cpy_files
            ]
...
def process_files(cpy_file: str, txt_file: str):
    ...
    try:
        ...
        if fields:
            ...
            if confirm(
                "\nWould you like to parse the data file using this structure?",
                default=True,
            ):
                print("\n🔄 Parsing data file...")
    ...
    try:
        ...
        for line in lines:
            ...
            field_match = re.match(
                r"\s*(\d{2})\s+([A-Z0-9-]+)(?:\s+REDEFINES\s+([A-Z0-9-]+))?(?:\s+PIC\s+([X9V()0-9]+))?\s*\.?",
                line,
                re.IGNORECASE,
            )
            ...
            if field_match:
                ...
                if level == 2:  # Level 02 - main record structures
                    if redefines_target:
                        ...
                        print(
                            f"🔄 REDEFINES detected: {name} redefines {redefines_target} at position 1"
                        )
                    else:
...
def display_cobol_structure(fields: List[CobolField]):
    ...
    for field in fields:
        ...
        print(
            f"{indent}{field.level:02d} {field.name}{pic_info}{length_info}{pos_info}"
        )
            ...
            for record_num, line in records[:2]:
                ...
                for field in field_list[:10]:  # Show first 10 fields
                    if field.start_pos <= len(line):
                        ...
                        if field.field_type == "numeric" and "V" in (
                            field.picture or ""
                        ):
                            field_value = format_numeric_field(
                                field_value, field.picture
                            )
...
def export_to_csv(
    parsed_data: Dict[str, Any], fields: List[CobolField], original_file: str
):
    ...
    try:
        ...
        custom_name = text_input(
            f"Output filename (default: {default_filename}):", default=default_filename
        )
        ...
        output_file = output_manager.get_output_path(
            "cobol_processor", output_filename, subfolder=""
        )
        ...
        if type_counts:
            ...
            for record_type, count in sorted(type_counts.items()):
                percentage = (
                    (count / records_written) * 100 if records_written > 0 else 0
                )
