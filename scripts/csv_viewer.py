...
class CSVViewerApp(App):
        ...
        with Horizontal():
            ...
            with Vertical(id="main-content"):
                ...
                with Horizontal(id="pagination-container"):
                    ...
                    yield Input(
                        value="1000", id="page-size-input", placeholder="Page size"
                    )
    ...
    def _update_table_content(self) -> None:
        ...
        visible_cols = [
            col for col in self.csv_data.columns if col in self.csv_data.visible_columns
        ]
    ...
    def _apply_filter_worker(self) -> None:
        ...
        if not filter_text:
            ...
            visible_cols = [
                ...
                if col in self.csv_data.visible_columns
            ]
            ...
            if visible_cols:
                ...
                for chunk_df in pd.read_csv(
                    ...
                    filtered_indices.extend(
                        [idx + start_idx for idx in matching_indices]
                    )

                self.csv_data.filtered_indices = (
                    filtered_indices if filtered_indices else []
                )
            else:
                self.csv_data.filtered_indices = []
    ...
    def _load_page_data(self) -> List[List[str]]:
        ...
        if self.csv_data.filtered_indices is not None:
            ...
            skip_rows = 1 + (
                self.csv_data.current_page * self.csv_data.page_size
            )  # +1 for header
    ...
    def _update_pagination_info(self) -> None:
        ...
        total_pages = max(
            1, (total_rows + self.csv_data.page_size - 1) // self.csv_data.page_size
        )
...
def main():
    ...
    separator = text_input(
        f"Enter CSV separator (detected: '{detected_sep}'):", default=detected_sep
    )
    if not separator:
