...
class ConsumerAPI:
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Auth credentials: {self.config.client_id}:{self.config.client_secret[:4]}***"
            )

            log_request(
                logger, "POST", self.config.auth_url, headers=headers, data=data
            )

            # Make the request with detailed logging
            logger.info("📡 Making POST request to Consumer API...")
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ Authentication successful, token expires in {expires_in}s"
                )
                logger.debug(
                ...
                if response.status_code == 403:
                    ...
                    logger.error(
                        "🔑 401 Unauthorized - Authentication credential issues:"
                    )
                ...
                logger.error(
                    f"   Auth: {self.config.client_id}:{self.config.client_secret[:4]}***"
                )
                ...
                logger.error(
                    f"     -H 'X-Consumer-Client-Id: {self.config.client_id}' \\"
                )
                logger.error(
                    "     -H 'Content-Type: application/x-www-form-urlencoded' \\"
                )
                logger.error(
                    f"     -u '{self.config.client_id}:{self.config.client_secret}' \\"
                )
                logger.error(
                    f"     -d 'grant_type=client_credentials&scope={self.config.scope}'"
                )
    ...
    def _query_debt_with_retry(self, credit_number: str, retry_count: int = 0) -> Dict:
        ...
        try:
            # Log request details
            log_request(logger, "GET", debt_url, headers=headers)

            response = requests.get(
                debt_url, headers=headers, timeout=self.config.timeout
            )
            ...
            if response.status_code in [401, 403] and retry_count < max_retries:
                logger.warning(
                    f"🔄 Authentication error {response.status_code} for {credit_number}, retrying..."
                )
                logger.info(
                    f"🔑 Re-authenticating (attempt {retry_count + 1}/{max_retries})"
                )
                ...
                if self.authenticate():
                    logger.info(
                        f"✅ Re-authentication successful, retrying query for {credit_number}"
                    )
            ...
            if response.status_code == 200:
                ...
                installments = result["response"].get(
                    "listRestInstallmentsPayableResponse", []
                )
                logger.info(
                    f"✅ Successfully queried {credit_number}: {len(installments)} installments"
                )
            else:
                logger.warning(
                    f"❌ Failed to query {credit_number}: status {response.status_code}"
                )
                try:
                    error_data = response.json()
                    if "errors" in error_data:
                        if (
                            isinstance(error_data["errors"], list)
                            and len(error_data["errors"]) > 0
                        ):
                            result["error"] = error_data["errors"][0].get(
                                "message", "Unknown error"
                            )
                        elif isinstance(error_data["errors"], dict):
                            result["error"] = error_data["errors"].get(
                                "message", "Unknown error"
                            )
                        else:
                    ...
                    logger.error(
                        f"Error details for {credit_number}: {result['error']}"
                    )
                except json.JSONDecodeError:
                    result["error"] = response.text
                    logger.error(
                        f"Raw error response for {credit_number}: {response.text}"
                    )
...
def get_processing_subset(credit_numbers: List[str]) -> List[str]:
    ...
    options = [
        ...
        {"name": "❌ Cancel", "value": "cancel", "description": "Cancel processing"},
    ]
    ...
    if selected["value"] == "all":
        ...
        while True:
            try:
                quantity_str = text_input(
                    f"Enter number of records to process (1-{total_count}):"
                )
                if not quantity_str:
                    return []
        ...
        with open(csv_file, "r", encoding="utf-8") as f:
            ...
            for i, col in enumerate(columns, 1):
                ...
                if any(
                    ...
                    for keyword in ["credit", "codigo", "ident", "loan", "credito"]
                ):
                    print(f"   {i}. {col} ⭐ (Potential credit code column)")
                elif any(
                    keyword in col.lower()
                    for keyword in ["numero", "number", "num", "id"]
                ):
            ...
            for i, col in enumerate(columns):
                ...
                column_options.append(
                    {"name": col, "value": col, "description": description}
                )
            ...
            selected_option = interactive_menu(
                "Select column for credit codes:", column_options
            )
...
def main():
    ...
    if not config:
        ...
        config_path = create_sample_script_config(
            "consumer_debt_checker", sample_config
        )
        print(f"⚠️  Created sample configuration file: {config_path}")
        print(
            "📝 Please edit the configuration file with your API credentials before continuing"
        )
        return

    # Validate required config
    if (
        not config.get("api", {}).get("client_id")
        or config.get("api", {}).get("client_id") == "YOUR_CLIENT_ID_HERE"
    ):
        print(
            "❌ Please configure your API credentials in config_consumer_debt_checker.yml"
        )
        return
    ...
    if not api_client.authenticate():
        ...
        print(
            "🔧 You can test the connection manually with option '🧪 Test API connection'"
        )
    ...
    if result["status_code"] == 200 and result["response"]:
        ...
        if installments:
            ...
            for inst in installments:
                ...
                print(
                    f"  Installment {receipt_number}: ${amount:,.0f} CLP (Due: {due_date})"
                )
...
def process_csv_file(config=None, api_client=None):
    ...
    print("\n🔍 Analyzing CSV file...")
    credit_numbers, original_data = extract_credit_numbers_from_csv_with_selection(
        csv_file
    )
    ...
    for i, credit_number in enumerate(credit_numbers, 1):
        ...
        if result["status_code"] == 200:
            installments = result.get("response", {}).get(
                "listRestInstallmentsPayableResponse", []
            )
...
def extract_credit_numbers_from_csv(csv_file: str) -> List[str]:
    ...
    logger.debug(
        f"Using delimiter: {default_delimiter} (fallback: {fallback_delimiter})"
    )
            ...
            if credit_number_column in first_line:
                ...
                for row_num, row in enumerate(reader, 2):  # Start from 2 (after header)
                    if row.get("RECORD_TYPE") == debt_record_type:
                        ...
                        if (
                            credit_number
                            and credit_number.isdigit()
                            and len(credit_number) >= min_credit_length
                        ):
                            credit_numbers.add(credit_number)
                            logger.debug(
                                f"Found credit number at row {row_num}: {credit_number}"
                            )
            else:
...
def get_delay_setting(config: Dict) -> float:
    ...
    delay_str = text_input(
        f"Enter delay between requests in seconds (default: {default_delay}):",
    ...
    try:
        ...
        logger.warning(
            f"Invalid delay value '{delay_str}', using default: {default_delay}"
        )
        return default_delay


def generate_results_csv(
    results: List[Dict], api: ConsumerAPI, original_data: List[Dict] = None
) -> str:
    ...
    output_file = output_manager.get_output_path(
        "consumer_debt_checker", filename, subfolder=""
    )
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for result in results:
            ...
            row.update(
                {
                    "error_category": (
                        api.categorize_error(result.get("error", ""))
                        if result.get("error")
                        else ""
                    ),
                ...
                if installments:
                    ...
                    for inst in installments:
                        ...
                        installment_details.append(
                            f"#{receipt_num}:${amount:,.0f}({due_date})"
                        )
...
def print_processing_summary(
    results: List[Dict], successful_count: int, output_file: str
):
