            ...
            for page_num, page in enumerate(pdf.pages, 1):
                try:
                    ...
                    if page_text and page_text.strip():
                        ...
                        text_content.append(f"--- Page {page_num} (No text found) ---\n")
                except Exception as e:
                    text_content.append(f"--- Page {page_num} (Error: {e}) ---\n")
...
def pdf_to_images_pdf2image(pdf_path: str, output_dir: str, format: str = 'PNG') -> List[str]:
...
def pdf_to_images_pymupdf(pdf_path: str, output_dir: str, format: str = 'PNG') -> List[str]:
    ...
    try:
        ...
        for page_num in range(len(doc)):
            ...
            pil_image = Image.open(tempfile.NamedTemporaryFile(suffix='.ppm', delete=False))
        ...
        raise Exception("PyMuPDF or Pillow not available. Install with: uv add PyMuPDF Pillow")

def convert_pdf_to_images(pdf_path: str, output_dir: str, format: str = 'PNG') -> List[str]:
    ...
    conversion_methods = [
        ...
        ('pymupdf', pdf_to_images_pymupdf, libraries['pymupdf'] and libraries['Pillow'])
    ]
...
def convert_pdf_file():
    ...
    if output_format == 'txt':
        ...
        # Convert to images
        output_dir = output_manager.get_output_path('pdf_converter', f"{base_name}_{timestamp}")
