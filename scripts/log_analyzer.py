...
class LogAnalyzer:
    ...
    def print_metrics(self) -> None:
        ...
        for i, (minute, count) in enumerate(top_minutes_of_hour, 1):
            percentage = (count / total_requests) * 100
            print(f"{i:2d}. Minute {minute:02d}: {count:,} requests ({percentage:.2f}%)")
    ...
    def export_to_json(self, output_file: str = None) -> None:
        ...
        results = {
            ...
            'top_metrics': {
                ...
                'top_10_hours_of_day': self.get_top_n(self.analyze_requests_by_hour_of_day(), 10),
                'top_10_minutes_of_hour': self.get_top_n(self.analyze_requests_by_minute_of_hour(), 10)
            }
        }
